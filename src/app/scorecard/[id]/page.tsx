...
import {
    ...
    type ScorecardClaim, type ChatTurn
} from "@/lib/supabase"
    ...
    return (
        <div className="space-y-6">
            ...
            {activeTab === "transcript" && (
                <div className="space-y-6">
                    {transcriptState === "loaded" && transcript && <TranscriptQA transcript={transcript} />}
...
function TranscriptQA({ transcript }: { transcript: string }) {
    ...
    async function ask(q: string) {
        ...
        setBusy(true)
        try {
            const answer = await askAboutTranscript(transcriptToText(transcript), question, history)
            setThread(t => [...t, { role: "assistant", content: answer }])
        } catch (e) {
            setError((e as Error).message || "Something went wrong.")
            setThread(t => t.slice(0, -1)) // drop the unanswered question
            setInput(question)
        } finally {
            setBusy(false)
        }
    ...
    return (
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2">
                <span className="text-sm font-semibold text-gray-900">Ask about this call</span>
